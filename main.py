...
proxies_list = [
    ...
    {"http": "http://3.129.184.210:3128", "https": "http://3.129.184.210:3128"},
    {"http": "http://143.110.226.180:8888", "https": "http://http://143.110.226.180:8888"},
...
async def get_servers(place_id, cursor=None, retries=480, initial_delay=0.25):
    ...
    while retries > 0:
        ...
        try:
            ...
            response = requests.get(url, proxies=proxy, timeout=1) if proxy else requests.get(url, timeout=1)
